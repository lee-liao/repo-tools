...
program.option("--owner <owner>", "The owner of the repository").option("--repo <repo>", "The name of the repository").option("--path <path>", "The path to the file within the repository").option("--ref <ref>", "The branch, tag, or commit SHA", "main").option("--output <output>", "Full output path including filename").option("--dest <dest>", "Destination folder (filename preserved from source)").parse(process.argv);
...
async function getSnippet() {
  try {
    const url2 = "http://103.98.213.149:8510/contents";
